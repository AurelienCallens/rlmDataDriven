                       ...
                       print.summary = TRUE) 
{
  ...
  if (phi.par == T) {
    phi <- median(abs(Y[-c(1:nlag)] - mu)/(var.func(mu, 
                                                    esti, phi = 1)))/0.6745
  }
  else {
    phi <- 1
  ...
  w_i <- as.vector((psi.Huber(pearson_res, c)/(pearson_res)))
  W <- pmin(as.vector(w_i/(var.func(mu, esti, phi)))^2, 1)
  B_est <- solve(crossprod(X[-c(1:nlag), ], W * X[-c(1:nlag), 
                                                  ]), crossprod(X[-c(1:nlag), ], W * Y[-c(1:nlag)]))
  mu <- X[-c(1:nlag), ] %*% B_est
  pearson_res <- (Y[-c(1:nlag)] - mu)/(var.func(mu, esti, 
                                                phi))
  p <- length(B_est)
  ...
  z$varpara <- list(var_func = var.function, gamma = esti, phi = phi)
  if (print.summary) {
    cat("Summary : \n ")
    print(z$summary)
    if (is.null(tuning.para)) {
